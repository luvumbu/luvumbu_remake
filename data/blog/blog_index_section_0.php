...
<script>
  ...
  class Bubble {
    ...
    reset() {
      this.z = 0.2 + Math.random() * 0.8; // profondeur entre 0.2 et 1
  ...
  // Palette de 16 couleurs très sombres (r,g,b)
  const darkColors = [
    { r: 3, g: 2, b: 8 },
    { r: 8, g: 4, b: 15 },
    { r: 2, g: 6, b: 9 },
  ...
  // Palette de couleurs Soleil/Terre lors du contact
  const contactColors = [
  ...
  let currentContactColorIndex = -1; // Pas de contact au début

  let currentIndex = 0;
  let nextIndex = 1;
  let transitionProgress = 0; // 0 à 1
  const transitionDuration = 1800; // 30 secondes à 60fps
  let framesSinceChange = 0;
  const framesBetweenChange = 1800; // 30 secondes pause avant changement
  ...
  function animate() {
    ...
    // Vérifier les collisions bulle-terre
